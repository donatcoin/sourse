...
const string strMessageMagic = "BotaniCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // BotaniCoin: 1 days
static const int64 nTargetSpacing = 120; // BotaniCoin: 2 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal botanicoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BotaniCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Botanicoin: 01.08.2017 - oo";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1496147701; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2275358;

        if (fTestNet)
        {
            block.nTime    = 1496145001;
            block.nNonce   = 291951;
        }
